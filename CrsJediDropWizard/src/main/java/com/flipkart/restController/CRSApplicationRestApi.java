...
public class CRSApplicationRestApi {
    ...
    public CRSApplicationRestApi(Validator validator) {
			...
			boolean loggedin=userInterface.verifyCredentials(userId, password);
				...
				if(loggedin)
				{
					...
					case STUDENT:
						...
						if(!isApproved)	
						{
							return Response.status(500).entity("Login unsuccessful! Student "+userId+" has not been approved by the administration!" ).build();
						}
						break;
					
						
					}
					return Response.status(200).entity("Login successful: "+ userId).build();
				}
				else
				{
